...
public:
	...
	GamePlayerData(int playerID, IBattleshipGameAlgo* inputPlayerAlgo, ShipsBoard inputShipsBoard,
		size_t shipsCount, int score =0, bool hasMoves = true) : id(playerID),playerAlgo(inputPlayerAlgo),
		shipsBoard(inputShipsBoard), currShipsCount(shipsCount),score(score), hasMoreMoves(hasMoves) {}
	~GamePlayerData();
