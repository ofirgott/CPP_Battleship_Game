...
public:
	...
	static void getAllCurrShipCoords(std::vector<char> board, int x, int y, int z, char currShipChar, std::set<Coordinate>& coordOfCurrentShip, int matRows, int matCols, int matDepth);
	...
	char& operator()(int x, int y, int z) {						
		...
		return boardVec.at(x + y * cols + z * cols * rows);
	}
private:
	...
	static void InitEmptyBoardVector(std::vector<char> boardVec, int rows, int cols, int depths);									 /* init already allocated matrix with ' ' */
